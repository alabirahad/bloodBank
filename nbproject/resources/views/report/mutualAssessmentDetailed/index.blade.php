                        ...
                        <table class="table table-bordered table-hover table-head-fixer-color" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="vcenter text-center" colspan="{{!empty($cmArr) ? sizeof($cmArr) : 1}}">@lang('label.CM_MARKING')</th>
                                    <th class="vcenter text-center" rowspan="2">@lang('label.AVG')</th>
                                ...
                                <tr>
                                    ...
                                    ?>
                                    <th class="vcenter text-center">{!! $cmName ?? '' !!}</th>
                                    @endforeach
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    ?>
                                    <td class="vcenter text-{{$alignment}}">{!! $markingPositionArr[$markingCmId][$cmId]['pos'] ?? '--' !!}</td>
                                    @endforeach
...
<script type="text/javascript">
    $(function () {
        ...
        $(document).on("change", "#termId", function () {
            ...
            $.ajax({
                url: "{{ URL::to('mutualAssessmentDetailedReport/getMaEvent')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("change", "#synId", function () {
            ...
            $.ajax({
                url: "{{ URL::to('mutualAssessmentDetailedReport/getsubSyn')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
