                            ...
                            <tr>
                                ...
                                <td class="text-center vcenter" width="50px">
                                    <?php if (!empty($assignedDs->photo && File::exists('public/uploads/user/' . $assignedDs->photo))) { ?>
                                        <img width="50" height="60" src="{{URL::to('/')}}/public/uploads/user/{{$assignedDs->photo}}" alt="{{ $assignedDs->full_name}}"/>
                                    <?php } else { ?>
                                        <img width="50" height="60" src="{{URL::to('/')}}/public/img/unknown.png" alt="{{ $assignedDs->full_name}}"/>
                                    <?php } ?>
                                </td>
                                <td class="vcenter">{{ $assignedDs->personal_no }}</td>
                                <td class="vcenter">{{ $assignedDs->rank_code }}</td>
                                <td class="vcenter">{{ $assignedDs->full_name }}</td>
