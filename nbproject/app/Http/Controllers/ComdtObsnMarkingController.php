...
class ComdtObsnMarkingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view('comdtObsnMarking.index')->with(compact('activeTrainingYearInfo', 'courseList'));
    }
    ...
    public function showCmMarkingList(Request $request) {
        ...
        // get position after comdt obsn
        $cmArr = Common::getPosition($cmArr, 'percent_after_comdt', 'position_after_comdt_obsn');

        // get comdt lock info
        $comdtObsnLockInfo = ComdtObsnMarkingLock::select('status')->where('course_id', $request->course_id)->first();


//        echo '<pre>';
//        print_r($ciObsnLockInfo);
//        exit;

        $html = view('comdtObsnMarking.showCmMarkingList', compact('assignedObsnInfo', 'gradeInfo'
                        , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr', 'comdtObsnLockInfo'
                        , 'ciObsnLockInfo'))->render();
