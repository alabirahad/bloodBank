...
class UnlockCiObsnMarkingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $targetArr = CiObsnMarkingLock::join('course', 'course.id', 'ci_obsn_marking_lock.course_id')
                ->join('users', 'users.id', 'ci_obsn_marking_lock.locked_by')
                ...
                , 'ci_obsn_marking_lock.id', 'ci_obsn_marking_lock.unlock_message', 'course.name as course_name');

        //begin filtering
        $searchCourse = $request->fil_course_id;
        if (!empty($searchCourse)) {
            $targetArr = $targetArr->where('ci_obsn_marking_lock.course_id', $searchCourse);
        }
        //end filtering
