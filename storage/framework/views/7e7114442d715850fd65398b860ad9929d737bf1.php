    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                ...
                <?php echo app('translator')->get('label.OBSN_MARKING'); ?>
                <?php else: ?>
                <?php echo app('translator')->get('label.COMDT_OBSN_MARKING'); ?>
                <?php endif; ?>
            </div>
        </div>
...
<script type="text/javascript">
    $(function () {
        ...
        $(document).on("change", "#courseId", function () {
            ...
            $.ajax({
                url: "<?php echo e(URL::to('comdtObsnMarking/showCmMarkingList')); ?>",
                type: "POST",
                dataType: "json",
                headers: {
        ...
        $(document).on('click', '.button-submit', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "<?php echo e(URL::to('comdtObsnMarking/saveObsnMarking')); ?>",
                        type: "POST",
                        datatype: 'json',
                        cache: false,
                        contentType: false,
...
//Rquest for unlock
        $(document).on('click', '.request-for-unlock', function (e) {
            ...
            $.ajax({
                url: "<?php echo e(URL::to('comdtObsnMarking/getRequestForUnlockModal')); ?>",
                type: "POST",
                datatype: 'json',
                cache: false,
                contentType: false,
        ...
        $(document).on('click', '.save-request-for-unlock', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "<?php echo e(URL::to('comdtObsnMarking/saveRequestForUnlock')); ?>",
                        type: "POST",
                        datatype: 'json',
                        cache: false,
                        contentType: false,
                        ...
                        success: function (res) {
                            $('.modal').modal('hide');
                            toastr.success(res, '<?php echo app('translator')->get("label.REQUEST_FOR_UNLOCK_HAS_BEEN_SENT_TO_SUPERADMIN_SUCCESSFULLY"); ?>', options);
                            $("#courseId").trigger('change');
                        },
                        error: function (jqXhr, ajaxOptions, thrownError) {
                            if (jqXhr.status == 400) {
...
<?php echo $__env->make('layouts.default.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\Xampp\htdocs\afwc\resources\views/comdtObsnMarking/index.blade.php ENDPATH**/ ?>